...
mkdir -p "$PIPELINE_ROOT" "$DATA_DIR" "$LOG_DIR"
...
if [[ -n "$BASELINE_ROLLOUT_DIR" ]]; then
    if [[ ! -f "$BASELINE_ROLLOUTS_JSONL" || "${REEXPORT_BASELINE_ROLLOUTS:-false}" == "true" ]]; then
        log "Stage 1/3: exporting existing Sokoban rollouts."
        python3 "$SCRIPT_DIR/export_baseline_rollouts.py" \
        ...
        log "Stage 1/3: reusing $BASELINE_ROLLOUTS_JSONL."
    fi
else
    ...
    if [[ "${SKIP_BASELINE_ROLLOUTS:-false}" == "true" ]]; then
        ...
        log "Stage 1/3: collecting ${NUM_TASKS} x ${ROLLOUTS_PER_TASK} visual Sokoban rollouts."
        env \
...
SKILL_MODEL="${SOKOBAN_SKILL_MODEL:-${SKILL_MODEL:-${OPENAI_MODEL:-}}}"
SKILL_BASE_URL="${SOKOBAN_SKILL_BASE_URL:-${OPENAI_BASE_URL:-}}"
SKILL_API_KEY="${SOKOBAN_SKILL_API_KEY:-${OPENAI_API_KEY:-}}"
if [[ -z "$SKILL_MODEL" ]]; then
    log "Set OPENAI_MODEL or SOKOBAN_SKILL_MODEL for episode-skill generation."
    exit 1
fi
...
log "Stage 2/3: generating Sokoban episode skills with $SKILL_MODEL."
skill_args=(
    --baseline-rollouts "$BASELINE_ROLLOUTS_JSONL"
    --output-dir "$PIPELINE_ROOT"
    --skill-gen-workers "${SOKOBAN_SKILL_GEN_WORKERS:-64}"
    --skill-model "$SKILL_MODEL"
    --skill-max-completion-tokens "${SOKOBAN_SKILL_MAX_COMPLETION_TOKENS:-$DEFAULT_SKILL_MAX_COMPLETION_TOKENS}"
    --skill-parse-attempts "${SOKOBAN_SKILL_PARSE_ATTEMPTS:-$DEFAULT_SKILL_PARSE_ATTEMPTS}"
)
...
if [[ "${SOKOBAN_INCLUDE_EPISODE_SUMMARY:-true}" != "true" ]]; then
    skill_args+=(--no-include-episode-summary)
fi
if [[ "${SOKOBAN_SKILL_RESUME:-true}" == "true" ]]; then
    skill_args+=(--resume)
fi
if [[ "${SOKOBAN_REGENERATE_CANDIDATES:-false}" == "true" ]]; then
    skill_args+=(--regenerate-candidates)
fi
if [[ "${SOKOBAN_MAX_CANDIDATES:-null}" != "null" ]]; then
    skill_args+=(--max-candidates "$SOKOBAN_MAX_CANDIDATES")
fi
...
log "Stage 3/3: building Sokoban visual episode-skill SFT data."
build_args=(
...
log "Sokoban SFT data pipeline finished: $SUMMARY_FILE"
